 ...
 */
export default function DashTimer(div) {
  var TAU = 2 * Math.PI,
    NAME = "DashTimer";
  ...
  self.setData = function(data, options) {
    ...
    dInfo_.svg = d3
      ...
      .attr(
        "transform",
        "translate(" + dash_.width / 2 + "," + dash_.height / 2 + ")"
      );
    ...
    dash_ = vanMerge_([
      {
        ...
        start: {
          // values at start of transition
          innerRatio: 0.8, // diameter of inner circle relative to height
          outerRatio: 0.95, // diameter of outer circle relative to height
        ...
        immediate: {
          // if true then transition is skipped
          angle: false // the angle
        },
        values: {
          // things to do with displaying values
          ...
          decorate: function(d) {
            // function to clean up data before displaying
            return Math.round(d);
          }
        },
  ...
  function work_(start, duration, finish) {
    ...
    show_(start, duration, finish).then(
      function(expired) {
        self.resolve();
      },
      function(stopped) {
  ...
  function show_(startAt, duration, finishAt) {
    (dash_.internal.paused = dash_.internal.finished = dash_.internal.cancelled = false),
      (dash_.internal.progress = 0);
    ...
    return new Promise(function(resolve, reject) {
      ...
      function rng(a, b) {
        return d3.interpolate(
          d3.interpolate(a, b)(startAt),
          d3.interpolate(a, b)(1)
        );
      }

      dInfo_.path
        .transition(dash_.name)
        .ease(dash_.ease)
        .duration(duration)
